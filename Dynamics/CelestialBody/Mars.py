...
class Mars(object):
    ...
    def update_state(self, current_jd, center_object_pos_i, center_object_vel_i,
        ...
        mars_ssb_pos, mars_ssb_vel = self.kernel_spk[0, 4].compute_and_differentiate(current_jd)  # vector from SSB 2 Mars
        if iscenter:
            return mars_ssb_pos, mars_ssb_vel
        self.current_pos_from_center_i = mars_ssb_pos * 1000 - center_object_pos_i   # km to m
        self.current_vel_from_center_i = mars_ssb_vel * 1000 / 86400.0 - center_object_vel_i # km/day to m/s
    ...
    def get_log_values(self):
        report = {'Mars_pos_i(X) [m]': np.array(self.historical_pos_from_center_i)[:, 0],
                  'Mars_pos_i(Y) [m]': np.array(self.historical_pos_from_center_i)[:, 1],
                  'Mars_pos_i(Z) [m]': np.array(self.historical_pos_from_center_i)[:, 2],
                  'Mars_vel_i(X) [m]': np.array(self.historical_vel_from_center_i)[:, 0],
                  'Mars_vel_i(Y) [m]': np.array(self.historical_vel_from_center_i)[:, 1],
                  'Mars_vel_i(Z) [m]': np.array(self.historical_vel_from_center_i)[:, 2],
                  'Mars_pos_b(X) [m]': np.array(self.historical_pos_from_sc_b)[:, 0],
                  'Mars_pos_b(Y) [m]': np.array(self.historical_pos_from_sc_b)[:, 1],
                  'Mars_pos_b(Z) [m]': np.array(self.historical_pos_from_sc_b)[:, 2],
                  'Mars_vel_b(X) [m]': np.array(self.historical_vel_from_sc_b)[:, 0],
                  'Mars_vel_b(Y) [m]': np.array(self.historical_vel_from_sc_b)[:, 1],
                  'Mars_vel_b(Z) [m]': np.array(self.historical_vel_from_sc_b)[:, 2],
                  'Mars_pos_sc_i(X) [m]': np.array(self.historical_pos_from_sc_i)[:, 0],
                  'Mars_pos_sc_i(Y) [m]': np.array(self.historical_pos_from_sc_i)[:, 1],
                  'Mars_pos_sc_i(Z) [m]': np.array(self.historical_pos_from_sc_i)[:, 2],
                  'Mars_vel_sc_i(X) [m]': np.array(self.historical_vel_from_sc_i)[:, 0],
                  'Mars_vel_sc_i(Y) [m]': np.array(self.historical_vel_from_sc_i)[:, 1],
                  'Mars_vel_sc_i(Z) [m]': np.array(self.historical_vel_from_sc_i)[:, 2]}
        return report
