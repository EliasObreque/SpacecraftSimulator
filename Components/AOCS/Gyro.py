...
class Gyro(ComponentBase):
    def __init__(self, port_id, properties, dynamics):
        ComponentBase.__init__(self, 100)
        ...
        self.historical_rw_c_tmp = []
        self.historical_nr_c_tmp = []
    ...
    def get_log_values(self, subsys):
        report = {'gyro_omega' + subsys + '_c(X)[rad/s]': np.array(self.historical_omega_c)[:, 0],
                  ...
                  'gyro_rw' + subsys + '_c(X)[rad/s]': np.array(self.historical_rw_c_tmp)[:, 0],
                  'gyro_rw' + subsys + '_c(Y)[rad/s]': np.array(self.historical_rw_c_tmp)[:, 1],
                  'gyro_rw' + subsys + '_c(Z)[rad/s]': np.array(self.historical_rw_c_tmp)[:, 2]}
        return report
    ...
    def log_value(self):
        ...
        self.historical_rw_c_tmp.append(self.n_rw_c.rn)
        self.historical_nr_c_tmp.append(self.nrs_c.nr)
